...
As mentioned above, we're limiting our analysis to years 2005 through 2023, instead of using the full inequality score data set (which starts in 1963).
...
For our linear regression analysis, we will use the averaged data set from the earlier scatter plot, which contains an average inequality index per country from 2005 to 2023 as well as an average happiness score per country from 2005 to 2023. 
...
augment(gapminder_average_lm) |> 
  mutate(var_fitted = var(.fitted),
         ...
         var_resid = var(.resid),
         r_square = sprintf("%.2f%%",var_fitted / var_resp)) |> 
...
Looking at our R-squared value, we can see that 2.09% of the variability in the response values is accounted for by our regression model. This is obviously a very small percentage of the overall variability, and thus, suggests that our model is not of very high quality. 
