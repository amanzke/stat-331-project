...
- **Happiness Score**: A measurement from 0 to 100, where subjects were asked to rank their current life satisfaction, where 0 is the worst possible life for the subject and 100 is the best based on the Cantril life ladder from the gapminder data [^1]. The happiness score is based on an the weighted average score using the country's population as weights. 
...
=======
1. We pivoted the data from a wide to long format which created new columns for year and score. This was necessary for joining the two data sets later on in our data cleaning process.
2. We then coonverted the year columns to integers, making the data more readable.
3. Next, we joined the datasets of happiness score and inequality index on the country and the year which combines our two longer data sets, ensuring only years with happiness scores (happiness starts in 2004, inequality starts in 1963).
4. Lastly, we removed any missing values, making it easier to plot our data later on. 
>>>>>>> Stashed changes
...
[2] WHO report on social determinants of health: https://www.who.int/publications/i/item/9789241563970
